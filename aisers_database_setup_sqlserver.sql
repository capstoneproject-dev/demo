-- ============================================================
-- AISERS Database Setup (SQL Server / SSMS)
-- Converted from: NEW NEW capstone_db.sql (MariaDB 10.4.32)
-- Generated: 2026-03-01
-- NOTE: No INSERT data included. Schema only.
-- ============================================================
-- Type Mapping Reference (MySQL -> SQL Server):
--   int(11)                  -> INT
--   varchar(N)               -> VARCHAR(N)
--   tinyint(1)               -> BIT
...
--   enum('1st','2nd')        -> VARCHAR(3) + CHECK constraint
--   BEFORE trigger           -> AFTER trigger (SQL Server has no BEFORE)
--   SIGNAL SQLSTATE '45000'  -> ROLLBACK + RAISERROR
-- ============================================================
...
CREATE TABLE dbo.academic_programs (
    ...
    CONSTRAINT pk_academic_programs PRIMARY KEY (program_id)
);
GO
...
CREATE TABLE dbo.org_roles (
    ...
    CONSTRAINT pk_org_roles PRIMARY KEY (role_id)
);
GO
...
CREATE TABLE dbo.organization_members (
    ...
    position_title NVARCHAR(120) NULL,
...
CREATE TABLE dbo.student_numbers (
    ...
    CONSTRAINT pk_student_numbers PRIMARY KEY (sn_id)
);
GO
...
CREATE TABLE dbo.pending_registrations (
    ...
    requested_position  VARCHAR(120) NULL,
    ...
    CONSTRAINT pk_pending_registrations PRIMARY KEY (reg_id)
);
GO

-- --------------------------------------------------------
-- Table: program_org_mappings
-- --------------------------------------------------------
CREATE TABLE dbo.program_org_mappings (
    ...
    CONSTRAINT pk_program_org_mappings PRIMARY KEY (mapping_id)
);
GO
...
CREATE TABLE dbo.attendance_records (
    ...
    CONSTRAINT pk_attendance_records PRIMARY KEY (record_id)
);
GO
...
CREATE TABLE dbo.document_submissions (
    ...
    custom_document_type VARCHAR(100)  NULL,
...
CREATE TABLE dbo.document_annotations (
    ...
    CONSTRAINT pk_document_annotations PRIMARY KEY (annotation_id)
);
GO

-- --------------------------------------------------------
-- Table: documents_approved
-- --------------------------------------------------------
CREATE TABLE dbo.documents_approved (
    ...
    CONSTRAINT pk_documents_approved PRIMARY KEY (repo_id),
...
CREATE TABLE dbo.inventory_categories (
    ...
    CONSTRAINT pk_inventory_categories PRIMARY KEY (category_id)
);
GO
...
CREATE TABLE dbo.inventory_items (
    ...
    overtime_interval_minutes INT           NULL,         -- Minutes per overtime block (e.g. 30). NULL = no overtime charging.
    overtime_rate_per_block   DECIMAL(10,2) NULL,         -- Fee per overtime block (e.g. 5.00). NULL = no overtime charging.
...
CREATE TABLE dbo.rental_items (
    ...
    overtime_interval_minutes INT           NULL,         -- Snapshot: overtime interval at rental time
    overtime_rate_per_block   DECIMAL(10,2) NULL,         -- Snapshot: overtime rate at rental time
    ...
    CONSTRAINT pk_rental_items PRIMARY KEY (rental_item_id)
);
GO
...
CREATE        INDEX fk_repo_approver                     ON dbo.documents_approved(approved_by_user_id);
GO

-- document_annotations
CREATE INDEX fk_doc_ann_submission       ON dbo.document_annotations(submission_id);
CREATE INDEX fk_doc_ann_user             ON dbo.document_annotations(created_by_user_id);
...
CREATE        INDEX fk_org_members_role  ON dbo.organization_members(role_id);
...
CREATE INDEX fk_reg_reviewer            ON dbo.pending_registrations(reviewed_by_user_id);
...
CREATE        INDEX fk_program_org_org        ON dbo.program_org_mappings(org_id);
CREATE        INDEX idx_program_org_map_program ON dbo.program_org_mappings(program_id, is_active);
GO

-- rentals
CREATE INDEX fk_rentals_processor    ON dbo.rentals(processed_by_user_id);
...
CREATE        INDEX fk_rental_items_item   ON dbo.rental_items(item_id);
...
CREATE        INDEX fk_sn_added_by                    ON dbo.student_numbers(added_by_user_id);
...
ALTER TABLE dbo.academic_programs
    ADD CONSTRAINT fk_programs_institute
        FOREIGN KEY (institute_id) REFERENCES dbo.institutes(institute_id);
GO
...
ALTER TABLE dbo.documents_approved
    ADD CONSTRAINT fk_repo_approver
        ...
        CONSTRAINT fk_repo_submission
            FOREIGN KEY (submission_id) REFERENCES dbo.document_submissions(submission_id);
GO

ALTER TABLE dbo.document_annotations
    ADD CONSTRAINT fk_doc_ann_submission
            FOREIGN KEY (submission_id)      REFERENCES dbo.document_submissions(submission_id) ON DELETE CASCADE,
        CONSTRAINT fk_doc_ann_user
            FOREIGN KEY (created_by_user_id) REFERENCES dbo.users(user_id);
GO

ALTER TABLE dbo.document_submissions
    ADD CONSTRAINT fk_doc_sub_org
            FOREIGN KEY (org_id)               REFERENCES dbo.organizations(org_id),
...
ALTER TABLE dbo.inventory_categories
    ADD CONSTRAINT fk_inventory_categories_org
        FOREIGN KEY (org_id) REFERENCES dbo.organizations(org_id) ON DELETE CASCADE;
GO

ALTER TABLE dbo.inventory_items
    ADD CONSTRAINT fk_inventory_category
            FOREIGN KEY (category_id)         REFERENCES dbo.inventory_categories(category_id),
        CONSTRAINT fk_inventory_org
            FOREIGN KEY (org_id)              REFERENCES dbo.organizations(org_id),
        CONSTRAINT fk_inventory_org_category
            FOREIGN KEY (org_id, category_id) REFERENCES dbo.inventory_categories(org_id, category_id);
GO

ALTER TABLE dbo.organization_members
    ADD CONSTRAINT fk_org_members_org
            FOREIGN KEY (org_id)          REFERENCES dbo.organizations(org_id) ON DELETE CASCADE,
        CONSTRAINT fk_org_members_org_role
            FOREIGN KEY (org_id, role_id) REFERENCES dbo.org_roles(org_id, role_id),
        CONSTRAINT fk_org_members_role
            FOREIGN KEY (role_id)         REFERENCES dbo.org_roles(role_id),
...
ALTER TABLE dbo.org_roles
    ADD CONSTRAINT fk_org_roles_org
        FOREIGN KEY (org_id) REFERENCES dbo.organizations(org_id) ON DELETE CASCADE;
GO

ALTER TABLE dbo.pending_registrations
    ADD CONSTRAINT fk_reg_reviewer
            FOREIGN KEY (reviewed_by_user_id) REFERENCES dbo.users(user_id) ON DELETE SET NULL,
        CONSTRAINT fk_reg_student_number
            FOREIGN KEY (student_number) REFERENCES dbo.student_numbers(student_number);
GO

ALTER TABLE dbo.program_org_mappings
    ADD CONSTRAINT fk_program_org_org
            FOREIGN KEY (org_id)     REFERENCES dbo.organizations(org_id)         ON DELETE CASCADE,
        CONSTRAINT fk_program_org_program
            FOREIGN KEY (program_id) REFERENCES dbo.academic_programs(program_id) ON DELETE CASCADE;
GO

ALTER TABLE dbo.rentals
    ADD CONSTRAINT fk_rentals_org
            FOREIGN KEY (org_id)               REFERENCES dbo.organizations(org_id),
        CONSTRAINT fk_rentals_processor
            FOREIGN KEY (processed_by_user_id) REFERENCES dbo.users(user_id),
        CONSTRAINT fk_rentals_renter
            FOREIGN KEY (renter_user_id)       REFERENCES dbo.users(user_id);
GO

ALTER TABLE dbo.rental_items
    ADD CONSTRAINT fk_rental_items_item
...
ALTER TABLE dbo.student_numbers
    ADD CONSTRAINT fk_sn_added_by
            FOREIGN KEY (added_by_user_id) REFERENCES dbo.users(user_id)             ON DELETE SET NULL,
        CONSTRAINT fk_student_numbers_institute
            FOREIGN KEY (institute_id)     REFERENCES dbo.institutes(institute_id),
        CONSTRAINT fk_student_numbers_program
            FOREIGN KEY (program_id)       REFERENCES dbo.academic_programs(program_id);
GO
...
-- NOTE: SQL Server has no BEFORE triggers.
--       All updated_at triggers use AFTER UPDATE.
--       TRIGGER_NESTLEVEL() > 1 prevents infinite recursion.
-- ============================================================
...
BEGIN
    ...
    FROM dbo.academic_programs t INNER JOIN inserted i ON t.program_id = i.program_id;
END;
GO
...
BEGIN
    ...
    IF TRIGGER_NESTLEVEL() > 1 RETURN;
    UPDATE t SET updated_at = SYSDATETIME()
    FROM dbo.announcements t INNER JOIN inserted i ON t.announcement_id = i.announcement_id;
END;
GO
...
BEGIN
    ...
    IF TRIGGER_NESTLEVEL() > 1 RETURN;
    UPDATE t SET updated_at = SYSDATETIME()
    FROM dbo.attendance_records t INNER JOIN inserted i ON t.record_id = i.record_id;
END;
GO

CREATE TRIGGER trg_document_annotations_updated_at ON dbo.document_annotations AFTER UPDATE AS
BEGIN
    SET NOCOUNT ON;
    IF TRIGGER_NESTLEVEL() > 1 RETURN;
    UPDATE t SET updated_at = SYSDATETIME()
    FROM dbo.document_annotations t INNER JOIN inserted i ON t.annotation_id = i.annotation_id;
END;
GO
...
BEGIN
    ...
    IF TRIGGER_NESTLEVEL() > 1 RETURN;
    UPDATE t SET updated_at = SYSDATETIME()
    FROM dbo.document_submissions t INNER JOIN inserted i ON t.submission_id = i.submission_id;
END;
GO
...
BEGIN
    ...
    IF TRIGGER_NESTLEVEL() > 1 RETURN;
    UPDATE t SET updated_at = SYSDATETIME()
    FROM dbo.events t INNER JOIN inserted i ON t.event_id = i.event_id;
END;
GO
...
BEGIN
    ...
    IF TRIGGER_NESTLEVEL() > 1 RETURN;
    UPDATE t SET updated_at = SYSDATETIME()
    FROM dbo.institutes t INNER JOIN inserted i ON t.institute_id = i.institute_id;
END;
GO
...
BEGIN
    ...
    IF TRIGGER_NESTLEVEL() > 1 RETURN;
    UPDATE t SET updated_at = SYSDATETIME()
    FROM dbo.inventory_categories t INNER JOIN inserted i ON t.category_id = i.category_id;
END;
GO
...
BEGIN
    ...
    IF TRIGGER_NESTLEVEL() > 1 RETURN;
    UPDATE t SET updated_at = SYSDATETIME()
    FROM dbo.inventory_items t INNER JOIN inserted i ON t.item_id = i.item_id;
END;
GO
...
BEGIN
    ...
    IF TRIGGER_NESTLEVEL() > 1 RETURN;
    UPDATE t SET updated_at = SYSDATETIME()
    FROM dbo.organizations t INNER JOIN inserted i ON t.org_id = i.org_id;
END;
GO
...
BEGIN
    ...
    IF TRIGGER_NESTLEVEL() > 1 RETURN;
    UPDATE t SET updated_at = SYSDATETIME()
    FROM dbo.organization_members t INNER JOIN inserted i ON t.membership_id = i.membership_id;
END;
GO
...
BEGIN
    ...
    IF TRIGGER_NESTLEVEL() > 1 RETURN;
    UPDATE t SET updated_at = SYSDATETIME()
    FROM dbo.org_roles t INNER JOIN inserted i ON t.role_id = i.role_id;
END;
GO

CREATE TRIGGER trg_pending_registrations_updated_at ON dbo.pending_registrations AFTER UPDATE AS
BEGIN
    SET NOCOUNT ON;
    IF TRIGGER_NESTLEVEL() > 1 RETURN;
    UPDATE t SET updated_at = SYSDATETIME()
    FROM dbo.pending_registrations t INNER JOIN inserted i ON t.reg_id = i.reg_id;
END;
GO
...
BEGIN
    ...
    IF TRIGGER_NESTLEVEL() > 1 RETURN;
    UPDATE t SET updated_at = SYSDATETIME()
    FROM dbo.program_org_mappings t INNER JOIN inserted i ON t.mapping_id = i.mapping_id;
END;
GO
...
BEGIN
    ...
    IF TRIGGER_NESTLEVEL() > 1 RETURN;
    UPDATE t SET updated_at = SYSDATETIME()
    FROM dbo.rentals t INNER JOIN inserted i ON t.rental_id = i.rental_id;
END;
GO
...
BEGIN
    ...
    IF TRIGGER_NESTLEVEL() > 1 RETURN;
    UPDATE t SET updated_at = SYSDATETIME()
    FROM dbo.rental_items t INNER JOIN inserted i ON t.rental_item_id = i.rental_item_id;
END;
GO

CREATE TRIGGER trg_student_numbers_updated_at ON dbo.student_numbers AFTER UPDATE AS
BEGIN
    SET NOCOUNT ON;
    IF TRIGGER_NESTLEVEL() > 1 RETURN;
    UPDATE t SET updated_at = SYSDATETIME()
    FROM dbo.student_numbers t INNER JOIN inserted i ON t.sn_id = i.sn_id;
END;
GO
...
BEGIN
    ...
    IF TRIGGER_NESTLEVEL() > 1 RETURN;
    UPDATE t SET updated_at = SYSDATETIME()
    FROM dbo.users t INNER JOIN inserted i ON t.user_id = i.user_id;
END;
GO
...
-- ------------------------------------------------------------
-- Mirrors: trg_doc_sub_to_repo  (MySQL AFTER UPDATE)
-- When a submission status changes to 'approved', upsert into
-- documents_approved. Uses MERGE as SQL Server equivalent of
...
BEGIN
    ...
    USING changed AS src ON tgt.submission_id = src.submission_id
    WHEN MATCHED THEN
        UPDATE SET
    ...
    WHEN NOT MATCHED BY TARGET THEN
        INSERT (submission_id, org_id, approved_by_user_id, title, document_type,
                file_url, description, semester, academic_year, approved_at)
        VALUES (
            src.submission_id,
            src.org_id,
            COALESCE(src.reviewed_by_user_id, src.submitted_by_user_id),
            src.title,
            src.document_type,
            src.file_url,
            src.description,
...
-- ------------------------------------------------------------
-- Mirrors: trg_rental_items_org_guard_insert  (MySQL BEFORE INSERT)
-- Ensures each rental item belongs to the same org as the rental.
-- ------------------------------------------------------------
...
BEGIN
    ...
    BEGIN
        ROLLBACK TRANSACTION;
        RAISERROR('Invalid rental item: inventory item belongs to a different organization.', 16, 1);
    END
END;
GO

-- ------------------------------------------------------------
-- Mirrors: trg_rental_items_org_guard_update  (MySQL BEFORE UPDATE)
-- ------------------------------------------------------------
...
BEGIN
    ...
    BEGIN
        ROLLBACK TRANSACTION;
        RAISERROR('Invalid rental item update: inventory item belongs to a different organization.', 16, 1);
    END
END;
GO

-- ------------------------------------------------------------
-- Mirrors: trg_users_set_institute_before_insert
--        + trg_users_set_institute_before_update  (MySQL BEFORE)
-- Automatically sets institute_id derived from program_id.
-- SQL Server: single AFTER INSERT, UPDATE trigger.
-- institute_id is nullable so the FK passes on insert;
-- the trigger then sets the correct value.
-- ------------------------------------------------------------
...
-- ============================================================
-- IDENTITY RESEED
-- Values match the AUTO_INCREMENT counters in the MySQL dump.
-- Remove or adjust these if starting with a completely empty DB.
-- ============================================================
